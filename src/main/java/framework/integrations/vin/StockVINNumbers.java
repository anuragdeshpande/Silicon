...
public class StockVINNumbers implements Serializable {
    ...
    private static void reloadVINs(){
        System.out.println("Getting 200 New Vin numbers for stock");
    ...
    private static void writeVins(List<String> vins){
        try(FileOutputStream fos = new FileOutputStream("vins.tmp"); ObjectOutputStream oos = new ObjectOutputStream(fos)){
    ...
    private static List<String> readVins(){
        try(FileInputStream fis = new FileInputStream("vins.tmp"); ObjectInputStream ois = new ObjectInputStream(fis)){
